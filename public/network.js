...
let network = null; // new null network instance
self.onmessage = function(e){ // On message event => handle the message
    switch(e.data.type){ // Switch on the data type
        case "create": // If the data type is create => create a new network
            network = new Network(e.data.pds); // Create a new network with the proper size => needs to be reworked since only the input layer size should be changed
            break; // Break the switch statement
        case "forward": // If the data type is forward => forward the input to the network
            network.giveInput(e.data.input); // give the input to the network
            let output = network.forward(); // forward the input through the network layers
            self.postMessage({type: "forward", output: output}); // post the output of the network
            break; // Break the switch statement
    }
}